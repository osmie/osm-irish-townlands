...
def search(request):
    ...
    baronies = list(Barony.objects.filter(qs).order_by("name"))
    baronies_num_results = len(baronies)
    civil_parishes = list(CivilParish.objects.filter(qs).order_by("name"))
    civil_parishes_num_results = len(civil_parishes)
    townlands = list(Townland.objects.filter(qs).order_by("name"))
