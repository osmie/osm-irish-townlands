...
psql -q -d gis -c "truncate table valid_polygon;" || true
psql -q -d gis -c "truncate table water_polygon;" || true
...
psql -d gis -c "create table if not exists water_polygon (way geometry(MultiPolygon, 900913), geo geography);"
psql -q -d gis -c "insert into water_polygon (way) select st_multi(way) from planet_osm_polygon where water IS NOT NULL OR waterway IS NOT NULL OR \"natural\" = 'water' and st_isvalid(way)";
...
psql -q -d gis -c "alter table valid_polygon add column geo geography;" || true
psql -q -d gis -c "update valid_polygon set geo = st_geographyfromtext(st_astext(st_transform(way, 4326)));"

psql -q -d gis -c "alter table water_polygon add column geo geography;" || true
psql -q -d gis -c "update valid_polygon set geo = st_geographyfromtext(st_astext(st_transform(way, 4326)));"
